...
local function ttf(font, size, x, y, text)
    text = text:gsub("'", "'\\''")
    fb("-t regular=" .. font .. ",size=" .. size .. " -x " .. (x or 0) .. " -y " .. (y or 0) .. " -h '" .. text .. "'")
end
...
local function clearStatus()
    fb("-k top=780,left=0,width=600,height=20")
    fb("-s top=780,left=0,width=600,height=20")
end
...
local function drawWifiIcon(up)
    if up then
        fb("-t regular=" .. F.sans_b .. ",size=12 -y 0 -X " .. WIFI_X .. " \"W\"")
    else
        fb("-c -p -x " .. WIFI_X .. " -y 0 -w 12 -h 1")
    end
end
...
while running do
    ...
    if needsData then
        ...
        if wifiOk then
            ...
            if shouldRedraw or touched then
                drawDashboard()
                wifiWasUp = false  -- redraw wifi icon after full draw overwrites it
            end
            enforceSettings()
        else
    ...
    -- Render clock locally between full syncs to avoid frequent server image downloads
